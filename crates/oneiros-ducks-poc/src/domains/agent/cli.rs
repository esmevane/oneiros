...
impl RemoteAgents {
    ...
    async fn request(
        ...
        let bytes = response.into_body().collect().await
            ...
            .to_bytes().to_vec();
        if status >= 400 {
            return Err(CliError::Status(status, String::from_utf8_lossy(&bytes).into_owned()));
        }
        Ok((status, bytes))
    }
    ...
    pub async fn list(&self) -> Result<AgentResponses, CliError> {
        let request = Request::builder().uri("/agents").body(Body::empty())
            .map_err(|e| CliError::Request(e.to_string()))?;
        let (_, bytes) = self.request(request).await?;
        serde_json::from_slice(&bytes).map_err(|e| CliError::Deserialize(e.to_string()))
    }
    ...
    pub async fn get(&self, name: &AgentName) -> Result<AgentResponses, CliError> {
        let request = Request::builder().uri(format!("/agents/{name}")).body(Body::empty())
            .map_err(|e| CliError::Request(e.to_string()))?;
        let (_, bytes) = self.request(request).await?;
        serde_json::from_slice(&bytes).map_err(|e| CliError::Deserialize(e.to_string()))
    }
