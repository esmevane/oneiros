...
## [0.0.12](https://github.com/esmevane/oneiros/compare/oneiros-engine-v0.0.11...oneiros-engine-v0.0.12) - 2026-09-09
...
- [patch] Employ noteworthy. (#305)

This commit introduces the second pass of our Noteworthy update from
PR #304, which basically takes the crate and uses it to annotate the
world so that we can clean up some of our documenation and resource
metadata info, and make it simpler to wrangle and maintain the varied
bits of details we use to create support features like our API docs,
skill definitions, and so on.

- [patch] DbHandler as an owned type. (#303)

This commit changes our `DbHandler` type into something which owns its
contents, so that we can distribute it in a way that doesn't mess up
call sites that need async closures. Essentially, we started with a ref
struct and wound up needing something a little more sophisticated, so
we're giving ourselves that, here.

- [patch] Database overhaul. (#302)

This is a pretty extensive update of our database operations meant
to improve ergonomics, make things mostly async by default, make it
more configurable, but most importantly: this commit introduces a
full memory mode for oneiros. This might fly in defiance of the idea
of a persistent continuity but it introduces an ephemeral capability
that lets us think of short term continuities (which we can still
pull and push from), I.E. enhancing portability modes in environments
that might be ephemeral but which we might want to know about agent
activity in.

The huge practical upshot of this, even without this mode of usage,
is that our test suite can now be ephemeral by default as well, which
means it's now extremely quick because we tossed much of the file IO.

- [patch] Testable config. (#301)

This commit dispatches with the free functions in the config test
suite and puts them into a harness. It also resolves an issue where
local test runs got flaky because of temp dir / env var mismatches.

- [patch] Tolerate refs in CLI. (#294)

We _did_ have this at one point so I think this is a bug fix on a
regression, but we want to be able to permit non-ref usage of the CLI.
We'll just assume it's there if it gets omitted.

Release-Type: fix

- [patch] Oh, database, you scoundrel. (#293)

We dove into test slowdown and found that the biggest culprit was our
handling of our database. It's always been sort of lacksadaisical by
design, but in a real sense it's begun to complicate the application and
slow down tests, specifically. Mainly, we're opening up connections a
ton, and doing a lot of boilerplate-like work around that.

The real fix is to actually reduce database operations to single cached
moments where we bootstrap a connection at maximum once, and then pool
it. But, we aren't there yet, architecturally - it'll take a thorough
databse refactor to get there. The upshot of doing that is we'll be able
to configure database modes and adapters, so we're definitely going
there, and soon. But, for now, we wanted to roll the test improvements
out on their own.
