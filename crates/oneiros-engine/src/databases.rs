...
impl Databases {
    ...
    /// Check out a bookmark's projection database (with events ATTACHed).
...
impl DbHandle {
    ...
    /// wrapper, not `rusqlite::Row`. Returns [`DbError::NotFound`] when
    /// no row matches.
...
mod tests {
    ...
    async fn bookmark_handle_has_events_attached() {
        ...
        let _events = databases
            .project_log(&ProjectName::new("alpha"))
            .await
            .unwrap();
        drop(_events);

        let bookmark = databases
            .bookmark(&ProjectName::new("alpha"), &BookmarkName::main())
            .await
            .unwrap();
    ...
    async fn sweep_drops_idle_connections() {
        ...
        let _handle = databases.host().await.unwrap();
        drop(_handle);
