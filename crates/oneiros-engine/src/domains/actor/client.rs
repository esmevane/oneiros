...
impl<'a> ActorClient<'a> {
    ...
    pub(crate) async fn get(&self, lookup: &GetActor) -> Result<ActorResponse, ClientError> {
        let GetActor::V1(lookup) = lookup;
        self.client.get(&format!("/actors/{}", lookup.key)).await
    }

    /// List all actors.
    pub(crate) async fn list(&self, listing: &ListActors) -> Result<ActorResponse, ClientError> {
