...
impl<'a> AgentClient<'a> {
    ...
    pub(crate) async fn list(&self, listing: &ListAgents) -> Result<AgentResponse, ClientError> {
    ...
    pub(crate) async fn get(&self, lookup: &GetAgent) -> Result<AgentResponse, ClientError> {
        let GetAgent::V1(lookup) = lookup;
        self.client.get(&format!("/agents/{}", lookup.key)).await
    }

    pub(crate) async fn update(&self, update: &UpdateAgent) -> Result<AgentResponse, ClientError> {
    ...
    pub(crate) async fn remove(&self, removal: &RemoveAgent) -> Result<AgentResponse, ClientError> {
