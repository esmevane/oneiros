...
impl<'a> BrainClient<'a> {
    ...
    pub(crate) async fn get(&self, lookup: &GetBrain) -> Result<BrainResponse, ClientError> {
        let GetBrain::V1(lookup) = lookup;
        self.client.get(&format!("/brains/{}", lookup.key)).await
    }

    /// List all brains.
    pub(crate) async fn list(&self, listing: &ListBrains) -> Result<BrainResponse, ClientError> {
