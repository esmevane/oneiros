...
impl Bridge {
    ...
    pub async fn confer(
        ...
        match response {
            SyncResponse::Updates { canon_bytes } => Ok(Some(canon_bytes)),
            SyncResponse::Current => Ok(None),
            SyncResponse::Events { .. } => Err(BridgeError::Protocol(
    ...
    pub async fn fetch_events(
        ...
        match response {
            ...
            SyncResponse::Current => Ok(Vec::new()),
            SyncResponse::Updates { .. } => Err(BridgeError::Protocol(
    ...
    async fn send(
        ...
        SyncResponse::from_bytes(&response_bytes)
