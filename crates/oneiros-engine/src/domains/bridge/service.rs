...
impl SyncHandler {
    ...
    async fn handle_confer(
        ...
        if canon_bytes.is_empty() {
            Ok(SyncResponse::Current)
        } else {
            Ok(SyncResponse::Updates { canon_bytes })
        }
    }
...
impl iroh::protocol::ProtocolHandler for SyncHandler {
    async fn accept(
        ...
        let response = match SyncRequest::from_bytes(&buf) {
            Ok(SyncRequest::Confer {
                link,
                version_vector,
            }) => self
                .handle_confer(&link, &version_vector)
                .await
                .unwrap_or_else(|e| SyncResponse::Denied {
                    reason: e.to_string(),
                }),
            Ok(SyncRequest::FetchEvents { link, event_ids }) => self
