...
impl ContinuityService {
    ...
    pub async fn emerge(
        ...
        let created =
            AgentService::create(context, name, persona, description, Prompt::new("")).await?;
        ...
        Self::wake(context, &agent_name, overrides).await?;
    ...
    pub async fn recede(
        ...
        AgentService::remove(context, name).await?;
    ...
    pub fn gather_context(
        ...
        let persona = PersonaRepo::new(&db).get(&persona_name)?;

        let agent_id_str = agent.id.to_string();

        // Vocabulary — system-wide
        let textures = TextureRepo::new(&db).list()?;
        let levels = LevelRepo::new(&db).list()?;
        let sensations = SensationRepo::new(&db).list()?;
        let natures = NatureRepo::new(&db).list()?;
        let urges = UrgeRepo::new(&db).list()?;

        // Memory filtering — core always included, rest filtered by level + capped
        let all_memories = MemoryRepo::new(&db).list(Some(&agent_id_str))?;
        ...
        let recent_experiences =
            ExperienceRepo::new(&db).list_recent(&agent_id_str, config.recent_window)?;
        ...
        while let Some((current, depth)) = frontier.pop_front() {
            ...
            let node_connections = ConnectionRepo::new(&db).list(Some(&ref_json))?;
        ...
        let mut cognitions = if connections.is_empty() {
            ...
            CognitionRepo::new(&db).list(Some(&agent_id_str), None)?
        } else {
        ...
        let raw_pressures = PressureRepo::new(&db).get(agent_name)?;
    ...
    fn assemble_cognitions(
        ...
        let agent_id_str = agent.id.to_string();
        let repo = CognitionRepo::new(db);

        // Recent cognitions for orientation
        let recent = repo.list_recent(&agent_id_str, config.recent_window)?;
    ...
    fn assemble_experiences(
        ...
        let repo = ExperienceRepo::new(db);
