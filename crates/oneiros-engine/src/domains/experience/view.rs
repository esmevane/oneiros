...
pub(crate) struct ExperienceView;

impl ExperienceView {
    /// Confirmation string for a successful create, using the response ref token.
    pub(crate) fn recorded(wrapped: &Response<Experience>) -> String {
        wrapped
            .meta()
            .ref_token()
            .map(|ref_token| {
    ...
    pub(crate) fn updated(wrapped: &Response<Experience>) -> String {
        wrapped
            .meta()
            .ref_token()
            .map(|ref_token| {
    ...
    pub(crate) fn table(items: &Listed<Response<Experience>>) -> Table {
    ...
    pub(crate) fn detail(item: &Experience) -> Detail {
