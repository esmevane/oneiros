...
impl<'a> LevelClient<'a> {
    ...
    pub(crate) async fn set(&self, setting: &SetLevel) -> Result<LevelResponse, ClientError> {
    ...
    pub(crate) async fn get(&self, lookup: &GetLevel) -> Result<LevelResponse, ClientError> {
        let GetLevel::V1(lookup) = lookup;
        self.client.get(&format!("/levels/{}", lookup.key)).await
    }

    pub(crate) async fn list(&self, listing: &ListLevels) -> Result<LevelResponse, ClientError> {
    ...
    pub(crate) async fn remove(&self, removal: &RemoveLevel) -> Result<LevelResponse, ClientError> {
