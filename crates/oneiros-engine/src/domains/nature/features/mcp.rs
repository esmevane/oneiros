...
pub fn dispatch(
    ...
    let value = match tool_name {
        "set_nature" => {
            ...
            let response =
                NatureService::set(ctx, nature).map_err(|e| ToolError::Domain(e.to_string()))?;
        ...
        "get_nature" => {
            let p: NameParam =
                serde_json::from_str(params).map_err(|e| ToolError::Parameter(e.to_string()))?;
            let response =
                NatureService::get(ctx, &p.name).map_err(|e| ToolError::Domain(e.to_string()))?;
            serde_json::to_value(response)
        }
        "list_natures" => {
            let response =
                NatureService::list(ctx).map_err(|e| ToolError::Domain(e.to_string()))?;
            serde_json::to_value(response)
        }
        "remove_nature" => {
            let p: NameParam =
                serde_json::from_str(params).map_err(|e| ToolError::Parameter(e.to_string()))?;
            let response = NatureService::remove(ctx, &p.name)
