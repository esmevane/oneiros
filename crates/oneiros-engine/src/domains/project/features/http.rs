...
async fn summary(context: ProjectContext) -> Result<Json<BrainSummary>, ProjectError> {
    ...
    let agents = AgentRepo::new(&db).list().unwrap_or_default();
    let agent_count = agents.len();

    let cognitions = CognitionRepo::new(&db).list(None, None).unwrap_or_default();
    ...
    let memory_count = MemoryRepo::new(&db).list(None).unwrap_or_default().len();
    let experience_count = ExperienceRepo::new(&db)
        .list(None)
        .unwrap_or_default()
        .len();
    let connection_count = ConnectionRepo::new(&db)
        .list(None)
        .unwrap_or_default()
        .len();
