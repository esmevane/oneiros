    ...
    pub async fn dispatch(
        ...
        let value = match tool_name {
            "set_sensation" => {
                ...
                let response = SensationService::set(context, sensation)
                    .await
            ...
            "get_sensation" => {
                let p: NameParam = serde_json::from_str(params)
                    .map_err(|e| ToolError::Parameter(e.to_string()))?;
                let response = SensationService::get(context, &p.name)
                    .map_err(|e| ToolError::Domain(e.to_string()))?;
                serde_json::to_value(response)
            }
            "list_sensations" => {
                let response = SensationService::list(context)
                    .map_err(|e| ToolError::Domain(e.to_string()))?;
                serde_json::to_value(response)
            }
            "remove_sensation" => {
                let p: NameParam = serde_json::from_str(params)
                    .map_err(|e| ToolError::Parameter(e.to_string()))?;
                let response = SensationService::remove(context, &p.name)
                    .await
                    .map_err(|e| ToolError::Domain(e.to_string()))?;
                serde_json::to_value(response)
            }
