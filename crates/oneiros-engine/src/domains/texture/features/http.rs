...
async fn set(
    ...
    Ok((
        StatusCode::OK,
        Json(TextureService::set(&context, texture).await?),
    ))
}

async fn list(context: ProjectContext) -> Result<Json<TextureResponse>, TextureError> {
    Ok(Json(TextureService::list(&context)?))
}
...
async fn show(
    ...
    Ok(Json(TextureService::get(&context, &name)?))
}
...
async fn remove(
    ...
    Ok(Json(TextureService::remove(&context, &name).await?))
}
