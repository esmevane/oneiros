...
pub fn dispatch(
    ...
    let value = match tool_name {
        "set_texture" => {
            ...
            let response =
                TextureService::set(ctx, texture).map_err(|e| ToolError::Domain(e.to_string()))?;
        ...
        "get_texture" => {
            let p: NameParam =
                serde_json::from_str(params).map_err(|e| ToolError::Parameter(e.to_string()))?;
            let response =
                TextureService::get(ctx, &p.name).map_err(|e| ToolError::Domain(e.to_string()))?;
            serde_json::to_value(response)
        }
        "list_textures" => {
            let response =
        ...
        "remove_texture" => {
            let p: NameParam =
                serde_json::from_str(params).map_err(|e| ToolError::Parameter(e.to_string()))?;
            let response = TextureService::remove(ctx, &p.name)
