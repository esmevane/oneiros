...
impl Server {
    ...
    pub(crate) async fn router_from_state(state: ServerState) -> Router {
        ...
        let router = ApiRouter::new()
            ...
            .merge(AgentRequest::router())
            .merge(BookmarkRequest::router())
            .merge(CognitionRequest::router())
            .merge(ConnectionRequest::router())
            .merge(ContinuityRequest::router())
            .merge(ExperienceRequest::router())
            .merge(FollowRequest::router())
            .merge(LevelRequest::router())
            .merge(MemoryRequest::router())
            .merge(NatureRequest::router())
            .merge(PeerRequest::router())
            .merge(PersonaRequest::router())
            .merge(PressureRequest::router())
            .merge(ProjectRequest::router().route("/summary", axum::routing::get(summary)))
            .merge(SearchRequest::router())
            .merge(SeedRequest::router())
            .merge(SensationRequest::router())
            .merge(
                StorageRequest::router()
                    .route("/storage/{ref_key}/content", axum::routing::get(content)),
            )
            .merge(HostRequest::router())
            .merge(LensRequest::router())
            .merge(SliceRequest::router())
            .merge(TenantRequest::router())
            .merge(TextureRequest::router())
            .merge(TicketRequest::router())
            .merge(TrailRequest::router())
            .merge(UrgeRequest::router())
            // Health check
            .api_route(
                "/health",
