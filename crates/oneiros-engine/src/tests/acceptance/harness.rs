...
pub(crate) trait Backend: Sized {
    ...
    fn exec_json(
        &self,
        command: &str,
    ) -> impl Future<Output = Result<crate::Responses, crate::Error>>;

    /// Execute in prompt mode — returns rendered text for content assertions.
    fn exec_prompt(&self, command: &str) -> impl Future<Output = Result<String, crate::Error>>;
}
...
impl<B: Backend> Harness<B> {
    ...
    /// harness.query("agent show viewer.process").assert_json(assert_response!(
