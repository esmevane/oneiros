...
async fn level_crud() {
    ...
    .unwrap();
    assert!(matches!(
        LevelService::get(&context, &LevelName::new("working")).unwrap(),
        LevelResponse::LevelDetails(_)
    ));

    match LevelService::list(&context).unwrap() {
    ...
    LevelService::remove(&context, &LevelName::new("working"))
        .await
        .unwrap();
    assert!(LevelService::get(&context, &LevelName::new("working")).is_err());
}
...
async fn persona_crud() {
    ...
    .unwrap();
    assert!(matches!(
        PersonaService::get(&context, &PersonaName::new("process")).unwrap(),
...
async fn agent_create_and_get() {
    ...
    match AgentService::get(&context, &AgentName::new("governor.test-persona")).unwrap() {
...
async fn cognition_add_and_list() {
    ...
    match CognitionService::list(&context, Some(AgentName::new("gov.test-persona")), None).unwrap()
    {
...
async fn replay_reconstructs_read_models() {
    ...
    assert!(matches!(
        AgentService::get(&context, &AgentName::new("gov.test-persona")).unwrap(),
    ...
    match AgentService::get(&context, &AgentName::new("gov.test-persona")).unwrap() {
...
async fn search_indexes_across_domains() {
    ...
    match SearchService::search(&context, "architecture", None).unwrap() {
        SearchResponse::Results(r) => assert_eq!(r.results.len(), 1),
    }

    // Agent itself should be indexed too (from seed_agent)
    match SearchService::search(&context, "Governor", None).unwrap() {
        SearchResponse::Results(r) => assert_eq!(r.results.len(), 1),
    }
...
async fn tenant_create_and_list() {
    ...
    match TenantService::list(&context).unwrap() {
...
async fn actor_create_and_get() {
    ...
    match ActorService::list(&context).unwrap() {
...
async fn brain_create_and_conflict() {
    ...
    match BrainService::get(&context, &BrainName::new("test-brain")).unwrap() {
...
async fn ticket_issue_and_validate() {
    ...
    // Validate the token
    match TicketService::validate(&context, token.as_str()).unwrap() {
...
async fn storage_upload_and_retrieve_content() {
    ...
    // Show by key
    match StorageService::show(&context, &StorageKey::new("test.txt")).unwrap() {
    ...
    // List
    match StorageService::list(&context).unwrap() {
    ...
    let retrieved = StorageService::get_content(&context, &StorageKey::new("test.txt")).unwrap();
    ...
    assert!(StorageService::show(&context, &StorageKey::new("test.txt")).is_err());

    // List should be empty
    assert!(matches!(
        StorageService::list(&context).unwrap(),
