...
#[tokio::test]
async fn collect_bookmark_from_remote() -> Result<(), Box<dyn core::error::Error>> {
    let remote = TestApp::new().await?.init_host().await?;
    let local = TestApp::new().await?.init_host().await?;
    ...
    local
        .command("bookmark collect their-feature --remote dreamforge --as my-copy")
        .await?;
...
/// Push to remote then collect back — roundtrip.
#[tokio::test]
async fn push_collect_roundtrip() -> Result<(), Box<dyn core::error::Error>> {
    let remote = TestApp::new().await?.init_host().await?;
    let local = TestApp::new().await?.init_host().await?;
    ...
    local
        ...
        .await?;
    local
        .command("bookmark collect on-remote --remote dreamforge --as pulled-back")
        .await?;
