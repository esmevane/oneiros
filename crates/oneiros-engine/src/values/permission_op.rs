...
///
/// - `BookmarkSubmit` — submit a bookmark to the remote (BridgeSubmitBookmark)
...
mod tests {
    ...
    fn serde_roundtrip_push() {
        let json = serde_json::json!("bookmark-submit");
        let decoded: PermissionOp = serde_json::from_value(json.clone()).unwrap();
        assert_eq!(decoded, PermissionOp::BookmarkSubmit);
