...
impl<T> HasPrompt<T> {
    pub fn new(prompt: impl Into<Prompt>, inner: T) -> Self {
        Self {
            prompt: prompt.into(),
            inner,
        }
...
impl<T> core::ops::Deref for HasPrompt<T> {
    type Target = T;
...
impl<T> Linkable for HasPrompt<T>
where
    T: Linkable,
{
...
mod tests {
    ...
    fn stores_id_with_shape() {
        ...
        let identity = HasPrompt::new("Super", widget);

        assert_eq!(identity.prompt, Prompt::new("Super"));
    ...
    fn different_identities_have_the_same_link() {
        ...
        let has_other_content = HasPrompt::new(
            "Duper",
            Actor {
                tenant_id,
                name: ActorName::new("red"),
