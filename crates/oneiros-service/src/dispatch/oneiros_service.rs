...
impl OneirosService {
    ...
    fn dispatch_data(&self, request: Requests) -> Result<Responses, Error> {
        Ok(match request {
            ...
            Requests::Actor(r) => self.dispatch_actor(r)?.into(),
            Requests::Brain(r) => self.dispatch_brain(r)?.into(),
            Requests::Tenant(r) => self.dispatch_tenant(r)?.into(),
            Requests::Ticket(r) => self.dispatch_ticket(r)?.into(),

            // Brain-scoped domains
            Requests::Agent(r) => self.dispatch_agent(r)?.into(),
            Requests::Cognition(r) => self.dispatch_cognition(r)?.into(),
            Requests::Connection(r) => self.dispatch_connection(r)?.into(),
            Requests::Dreaming(r) => self.dispatch_dream(r)?.into(),
            Requests::Event(r) => self.dispatch_event(r)?.into(),
            Requests::Experience(r) => self.dispatch_experience(r)?.into(),
            Requests::Introspecting(r) => self.dispatch_introspect(r)?.into(),
            Requests::Level(level_requests) => LevelStore
                .dispatch(RequestContext::new(level_requests, &self.brain_scope()?))?
                .into(),
            Requests::Lifecycle(r) => self.dispatch_lifecycle(r)?.into(),
            Requests::Memory(r) => self.dispatch_memory(r)?.into(),
            Requests::Nature(r) => self.dispatch_nature(r)?.into(),
            Requests::Persona(r) => self.dispatch_persona(r)?.into(),
            Requests::Pressure(r) => self.dispatch_pressure(r)?.into(),
            Requests::Reflecting(r) => self.dispatch_reflect(r)?.into(),
            Requests::Search(r) => self.dispatch_search(r)?.into(),
            Requests::Sensation(r) => self.dispatch_sensation(r)?.into(),
            Requests::Sense(r) => self.dispatch_sense(r)?.into(),
            Requests::Storage(r) => self.dispatch_storage(r)?.into(),
            Requests::Texture(r) => self.dispatch_texture(r)?.into(),
            Requests::Urge(r) => self.dispatch_urge(r)?.into(),
        })
    }
