...
pub(crate) async fn pressure_prompt_contains_readings<B: Backend>() -> TestResult {
    ...
    let prompt = harness.exec_prompt("pressure thinker.process").await?;
...
pub(crate) async fn returns_readings_for_agent<B: Backend>() -> TestResult {
    ...
    let response = harness.exec_json("pressure thinker.process").await?;
...
pub(crate) async fn introspect_pressure_decreases_after_introspecting<B: Backend>() -> TestResult {
    ...
    let before_response = harness.exec_json("pressure thinker.process").await?;
